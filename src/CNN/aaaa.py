    ...
    def fit(self, x, y, learning_rate=0.2, epochs=10000):  # X二维的,一行代表一个实例  Y 输出值
        ...
        for k in range(epochs):
            ...
            for j in range(len(self.weights)):  # going forward network, for each layer
                out = np.dot(out_list[j], self.weights[j]) + self.biases[j]
                out_list.append(self.activation(out))

            #             out = outList[-1]
            deltas = [out_list[-1] * (1 - out_list[-1]) * (y[i] - out_list[-1])]
            length = len(self.weights)
            length2 = len(out_list)
            for j in range(length):  #
                temp = out_list[j].dot(1 - out_list[j]).dot(deltas[j].dot(self.weights[length - j - 1].T))
                deltas.append(temp)
